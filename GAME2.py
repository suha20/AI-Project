import sys, pygame, math, numpy as np,random, matplotlib.pyplot as plt,time



#For efficient run of the program and to avoid time delay ,choose between level 1-5
...
def window_evaluation(window,piece,score) :
    score = 0
...
def get_score(board,piece):
    ...
    for r in range(ROW_COUNT):
        ...
        for c in range(COLUMN_COUNT-3):
            window = row_array[c:c+window_range]

            score += window_evaluation(window,piece,score)

    # SCORE- VERTICALLY
    for c in range(COLUMN_COUNT):
        col_array = [int(i) for i in list(board[:,c])]
        for r in range(ROW_COUNT-3):
            window = col_array[r:r+window_range]
    ...
    for r in range(ROW_COUNT-3):
        for c in range(COLUMN_COUNT-3):
            window = [board[r+i][c+i] for i in range(window_range)]
    ...
    for r in range(ROW_COUNT - 3):
        for c in range(COLUMN_COUNT - 3):
            window = [board[r - 3 + i][c - i] for i in range(window_range)]
...
def MINIMAX_with_alpha_beta_pruning(board, Difficulty_level,alpha,beta, maximizingplayer):
    valid_locations = get_valid_locations(board)
    ...
    if maximizingplayer:
        ...
        for col in valid_locations : #possible moves to make
            ...
            new_value = MINIMAX_with_alpha_beta_pruning(new_board,Difficulty_level-1,alpha,beta,False)[1]


            if new_value > value:
                value, best_max_column = new_value, col
        ...
        for col in valid_locations:  # child nodes/possible moves to make
            ...
            new_value1 = MINIMAX_with_alpha_beta_pruning(new_board, Difficulty_level - 1,alpha,beta, True)[1]

            if new_value1 < value :
                value, best_min_column = new_value1, col
...
while not game_over:
    ...
    if turn == player:
        ...
        label = myfont2.render("Player's turn", 1, RED)

    else:
        pygame.draw.circle(screen, YELLOW, (posx, int(square_size / 2)), RADIUS)
        label = myfont2.render("AI's turn", 1, YELLOW)
    ...
    if turn == AI and not game_over:
        col,max_score = MINIMAX_with_alpha_beta_pruning(board, Difficulty_level, -math.inf, math.inf ,True )
...
def measure_time_complexity(board, Difficulty_level, alpha, beta, maximizingplayer):
    start_time = time.time()
    MINIMAX_with_alpha_beta_pruning(board, Difficulty_level, alpha, beta, maximizingplayer)
    end_time = time.time()
    return end_time - start_time
# Example: Measure time complexity for different depths
Difficulty_level_to_test = [1,2,5,6,10,19,30]  # Add more depths as needed
time_complexity_results = []

for depth in Difficulty_level_to_test:
    board = create_board()
    # Assuming you want to measure the time for AI's turn
    time_taken = measure_time_complexity(board, Difficulty_level, -math.inf, math.inf, True)
    time_complexity_results.append((Difficulty_level, time_taken))

# Plotting the results using matplotlib


Difficulty_level, times = zip(*time_complexity_results)

plt.plot(Difficulty_level, times, marker='o')
plt.title('Minimax with Alpha-Beta Pruning Time Complexity Analysis')
